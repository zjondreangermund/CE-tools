...
Write-Host "Preparing the current Master Items Phase 8 source..." -ForegroundColor Cyan
...
if ([string]::IsNullOrWhiteSpace($AutoCADRoot)) {
    ...
    & $baseBuild -Version $Version -Configuration $Configuration -AutoCADRoot $AutoCADRoot
}

if ($LASTEXITCODE -ne 0) { throw "The current Master Items Phase 8 build failed." }
Write-Host "Current Master Items Phase 8 build completed." -ForegroundColor Green
