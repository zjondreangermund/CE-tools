...
namespace CETools.Civil3D
{
    ...
    internal static class SurfaceComparisonLinkStore
    {
        ...
        public static int RefreshAll(Document document)
        {
            ...
            using (Transaction transaction = database.TransactionManager.StartTransaction())
            {
                foreach (ObjectId entityId in ReadEntityIds(database, transaction))
                {
                    ...
                    if (entity != null &&
                        !(entity is Table) &&
                        Math.Abs(result.ComparisonZ - link.Point.Z) > Tolerance)
                    {
                        try
                        {
                            entity.TransformBy(Matrix3d.Displacement(
                                new Vector3d(
                                    0.0,
                                    0.0,
                                    result.ComparisonZ - link.Point.Z)));
                        }
                        catch
                        {
                            // Text still refreshes even where a host object cannot move.
                        }
                    }
                    ...
                    WriteRecord(
                        ...
                        link.BaseSurfaceId,
                        link.ComparisonSurfaceId,
                        new Point3d(link.Point.X, link.Point.Y, result.ComparisonZ));
                    if (updated) changed++;
                }
                transaction.Commit();
            }
            ...
            double textHeight)
        {
            string[] headings =
            {
                "Base Surface", "Comparison Surface", "X", "Y",
                "Base Z", "Comparison Z", "Difference", "Result"
            };
            string[] values =
            {
                result.BaseName,
                ...
                result.BaseZ.ToString("N3", CultureInfo.CurrentCulture),
                result.ComparisonZ.ToString("N3", CultureInfo.CurrentCulture),
                result.Difference.ToString("N3", CultureInfo.CurrentCulture),
            ...
            {
                table.Columns[column].Width = textHeight *
                    Math.Max(11.0, Math.Min(24.0, headings[column].Length + 4.0));
        ...
        private static ComparisonResult ReadResult(
            Database database,
            ...
            Point3d point)
        {
            CivilSurface baseSurface = transaction.GetObject(
                baseSurfaceId,
                OpenMode.ForRead,
                false) as CivilSurface;
            CivilSurface comparisonSurface = transaction.GetObject(
                comparisonSurfaceId,
                OpenMode.ForRead,
                false) as CivilSurface;
            if (baseSurface == null || comparisonSurface == null)
                throw new InvalidOperationException("A linked surface is unavailable.");
            ...
            return new ComparisonResult(
                baseSurface.Name,
                comparisonSurface.Name,
                new Point3d(point.X, point.Y, comparisonZ),
                baseZ,
                comparisonZ);
        }
        ...
        private static string BuildContents(ComparisonResult result)
        {
            return string.Join(
                "\\P",
                result.BaseName + " → " + result.ComparisonName,
                "BASE Z: " + result.BaseZ.ToString("N3", CultureInfo.CurrentCulture),
                "COMPARISON Z: " + result.ComparisonZ.ToString("N3", CultureInfo.CurrentCulture),
                "DIFF: " + result.Difference.ToString("N3", CultureInfo.CurrentCulture),
                result.Classification);
        }
        ...
        {
            public LinkData(ObjectId baseId, ObjectId comparisonId, Point3d point)
            {
        ...
        private sealed class ComparisonResult
        {
                ...
                double comparisonZ)
            {
                ...
                Classification = Difference > 0.0005
                    ? "Fill / raise"
