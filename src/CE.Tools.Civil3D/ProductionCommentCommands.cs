...
{
    /// <summary>
    /// Consolidated launchers for active BOQ, report, drawing-book and printing
    /// comments. Existing linked CE workflows remain the source of truth while
    /// these windows make every output and refresh action discoverable together.
    /// </summary>
    public sealed class ProductionCommentCommands
    {
        ...
        private static void RunChoiceWindow(
            string title,
            string subtitle,
            IList<ProductionChoice> choices)
        {
            Document document = ActiveDocument();
            if (document == null) return;
    ...
    internal sealed class ProductionChoiceWindow : Window
    {
        ...
        public ProductionChoiceWindow(
            string title,
            string subtitle,
            IEnumerable<ProductionChoice> choices)
        {
            Title = title;
            Width = 720;
            Height = 580;
