...
namespace CETools.Civil3D
{
    ...
    internal static class DynamicCoordinateLinkStore
    {
        ...
        private const string SurfaceElevationRecordName = "CE_DYNAMIC_SURFACE_ELEVATION";
        private const string PointNameRecordName = "CE_DYNAMIC_POINT_NAME";
        ...
        public static void LinkFeatureLineVertices(
            Database database,
            ObjectId featureLineId,
            IList<ObjectId> pointIds)
        {
            // The persisted record is intentionally shared with polyline vertices:
            // both relationships are source-handle + stable vertex index links.
            LinkPolylineVertices(database, featureLineId, pointIds);
        }

        public static void LinkFeatureLineVertex(
            Database database,
            ObjectId featureLineId,
            ObjectId pointId,
            int vertexIndex)
        ...
        public static void LinkSurfaceElevation(
            Database database,
            ObjectId surfaceId,
            IEnumerable<ObjectId> pointIds)
        {
            ...
            using (Transaction transaction = database.TransactionManager.StartTransaction())
            {
                foreach (ObjectId pointId in pointIds)
                {
                    ...
                    try
                    {
                        value = transaction.GetObject(pointId, OpenMode.ForWrite, false);
                    }
                    catch
                    {
                        continue;
            ...
            string pointName)
        {
            ...
            using (Transaction transaction = database.TransactionManager.StartTransaction())
            {
                DBObject value = transaction.GetObject(
                    ...
                    OpenMode.ForWrite,
                    false);
                var cogoPoint = value as CivilCogoPoint;
                if (cogoPoint != null && !string.IsNullOrWhiteSpace(pointName))
                {
                    try
                    {
                        cogoPoint.PointName = pointName.Trim();
                        cogoPoint.RawDescription = pointName.Trim();
                    }
                    catch
                    {
                        // Some drawings enforce point-name uniqueness. The
                        // extension record remains the authoritative fallback.
                    }
                }
                Xrecord record = GetOrCreateRecord(
                    value,
                ...
                if (record != null)
                {
                    record.Data = new ResultBuffer(
                        ...
                        new TypedValue(
                            (int)DxfCode.Text,
                            "PointName=" + (pointName ?? string.Empty).Trim()));
                }
                transaction.Commit();
            }
        }
            ...
            string fallback)
        {
            ...
            if (TryReadRecord(
                    ...
                    PointNameRecordName,
                    out values) &&
                values.TryGetValue("PointName", out pointName) &&
        ...
        public static int Refresh(Document document)
        {
                ...
                database.TransactionManager.StartTransaction())
            {
                ...
                // Re-sample linked points after their XY location changes. This
                // keeps Z correct between contour lines as well as directly on
                // contours; points outside the selected surface retain their
                // current elevation instead of being forced to zero.
                foreach (ObjectId entityId in entities)
                {
                    DBObject value;
                    try
                    ...
                    ObjectId surfaceId;
                    if (!TryReadSurfaceElevationRecord(
                        database,
                        value,
                        transaction,
                        out surfaceId))
                    ...
                    try
                    {
                        double elevation = surface.FindElevationAtXY(
                            current.X,
                            current.Y);
                        if (double.IsNaN(elevation) ||
                            double.IsInfinity(elevation) ||
                            Math.Abs(elevation - current.Z) <= Tolerance)
                        {
                            continue;
                        }
                        if (TrySetCoordinate(
                            value,
                            new Point3d(current.X, current.Y, elevation)))
                        {
                            changed++;
                        }
                    }
                    ...
                    {
                        // Outside the surface boundary: preserve the existing Z.
                    }
                }
                ...
                database.TransactionManager.StartTransaction())
            {
                foreach (ObjectId entityId in ReadAllEntityIds(database, transaction))
                {
                    ...
                    if (HasRecord(value, transaction, FollowerRecordName) ||
                        ...
                        HasRecord(value, transaction, SurfaceElevationRecordName) ||
                        HasRecord(value, transaction, PointNameRecordName))
                    {
                        count++;
                    }
                }
        ...
        private static bool TryReadSurfaceElevationRecord(
            Database database,
            DBObject target,
            Transaction transaction,
            out ObjectId surfaceId)
        {
            ...
            string handleText;
            if (!values.TryGetValue("Surface", out handleText) ||
                !TryResolveHandle(database, handleText, out surfaceId))
            {
                return false;
            }
            return !surfaceId.IsNull && !surfaceId.IsErased;
        }
            ...
            out string pointName)
        {
            ...
            if (cogo != null)
            {
                ...
                pointName = ReadPointName(
                    cogo,
                    transaction,
                    string.IsNullOrWhiteSpace(cogo.PointName)
            ...
            Transaction transaction)
        {
            ...
            var featureLine = source as FeatureLine;
            if (featureLine != null)
            {
