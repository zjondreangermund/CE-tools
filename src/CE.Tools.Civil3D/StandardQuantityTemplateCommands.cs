...
namespace CETools.Civil3D
{
    ...
    public sealed class StandardQuantityTemplateCommands
    {
            ...
            StandardQuantityTotals totals)
        {
            ...
            {
                lines.Add(Line("Surfacing", "80 mm 35 MPa interlocks", "m²", area, "Dynamic area sources"));
                lines.Add(Line("Bedding", "20 mm sand", "m³", area * link.SandThickness, "Dynamic area × thickness"));
                lines.Add(Line("Layerworks", "150 mm subbase - G5 @ 95% MOD AASHTO", "m³", area * link.SubbaseThickness, "Dynamic area × thickness"));
                lines.Add(Line("Layerworks", "150 mm selected subgrade - G6 @ 93% MOD AASHTO", "m³", area * link.SelectedThickness, "Dynamic area × thickness"));
                lines.Add(Line("Layerworks", "150 mm roadbed - rip and recompact to 90% MOD AASHTO", "m³", area * link.RoadbedThickness, "Dynamic area × thickness"));
                lines.Add(Line("Earthworks", "Cut (up to assembly datum)", "m³", link.CutVolume, "Linked/entered volume"));
                lines.Add(Line("Earthworks", "Fill - G9 @ 90% MOD AASHTO (up to assembly datum)", "m³", link.FillVolume, "Linked/entered volume"));
            ...
            StandardQuantityTemplate template)
        {
            ...
            double[] widths = { 6.0, 14.0, 34.0, 7.0, 13.0, 28.0 };
            for (int column = 0; column < columns; column++)
                table.Columns[column].Width = Math.Max(height * widths[column], 0.001);
