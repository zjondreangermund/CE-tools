...
{
    /// <summary>
    /// Survey/geospatial utilities. Geographic conversion uses the DWG's
    /// AutoCAD GeoLocationData transform; coordinate-system choices come from
    /// Civil 3D. Google imagery is never scraped: image import accepts a local
    /// Google Earth KML/KMZ GroundOverlay supplied by the user.
    /// </summary>
    public sealed class SurveyGoogleEarthCommands
    {
        [CommandMethod("CE_TOOLS", "CE_SURVEYLOCATIONAUTO", CommandFlags.Modal | CommandFlags.Redraw)]
        public void ShowSurveyLocation()
        {
            ...
            ObjectId boundaryId = SelectClosedPolyline(
                document.Editor,
            ...
            if (!ReadBoundary(document.Database, boundaryId, out boundary, out centroid))
            {
                document.Editor.WriteMessage(
                    "\nCE_SURVEYLOCATIONAUTO: a valid closed lightweight polyline is required.");
                return;
            }

            string[] coordinateCodes = ReadCoordinateCodes();
            string currentCode = ReadDrawingCoordinateSystem(civilDocument);
            SurveyLocationInfo info = BuildSurveyInfo(
                document.Database,
                currentCode,
                centroid,
                boundary);
            ...
            while (true)
            {
                using (var form = new SurveyLocationForm(info, coordinateCodes))
                {
                    ...
                    if (form.Action == SurveyDialogAction.Close ||
                        form.Action == SurveyDialogAction.None)
                        return;
                    ...
                    if (form.Action == SurveyDialogAction.ApplyCoordinateSystem)
                    {
                        ...
                        string message;
                        if (!ApplyCivilCoordinateSystem(civilDocument, requested, out message))
                        {
                            MessageBox.Show(
                                message,
                    ...
                    if (form.Action == SurveyDialogAction.OpenAutodeskCoordinateSelector)
                    {
                        document.SendStringToExecute("_.GEOCSASSIGN ", true, false, true);
                        return;
                    }
                    ...
                    {
                        MessageBox.Show(
                            "The selected polyline cannot yet be transformed to longitude/latitude. Assign/confirm the drawing coordinate system and AutoCAD geolocation data first. Use the Autodesk CRS button if required, then run this command again.",
                            "CE Tools - Survey Location",
                            MessageBoxButtons.OK,
                            MessageBoxIcon.Warning);
                        continue;
        ...
        public void ImportGoogleEarthImage()
        {
            ...
            {
                editor.WriteMessage(
                    "\nCE_GOOGLEEARTHIMAGE: the drawing needs valid AutoCAD geolocation data. Run CE_SURVEYLOCATIONAUTO to inspect the location/CRS first.");
                return;
            }

            PromptKeywordOptions openOptions = new PromptKeywordOptions(
                "\nOpen the selected boundary in Google Earth before choosing the overlay file? [Yes/No] <Yes>: ");
            ...
            if (openResult.Status == PromptStatus.None ||
                string.Equals(openResult.StringResult, "Yes", StringComparison.OrdinalIgnoreCase))
            {
                string kml = WriteTemporaryBoundaryKml(info);
                OpenGoogleEarth(kml, info.CentreWgs84);
            }
            ...
            if (!TryWgs84ToDrawing(
                document.Database,
                overlay.Wgs84Corners,
                out drawingCorners))
            {
                editor.WriteMessage(
                    "\nCE_GOOGLEEARTHIMAGE: Google Earth coordinates could not be transformed into this drawing. Confirm the DWG geolocation/coordinate system.");
                return;
            }
            ...
            try
            {
                ...
                editor.Regen();
                if (!imageId.IsNull)
                    editor.SetImpliedSelection(new[] { imageId });
                editor.WriteMessage(
                    "\nCE_GOOGLEEARTHIMAGE: image attached, georeferenced/oriented from the GroundOverlay and clipped to the selected polyline.");
            }
            catch (System.Exception exception)
            {
                editor.WriteMessage(
                    "\nCE_GOOGLEEARTHIMAGE failed. {0}",
                    exception.Message);
            }
        }
        ...
        private static bool ReadBoundary(
            Database database,
            ObjectId id,
        ...
        private static List<Point3d> TessellatePolyline(
            Polyline polyline,
            double maxAngle)
        {
        ...
        private static string ReadDrawingCoordinateSystem(CivilDocument civilDocument)
        {
            try
            {
                string code = civilDocument.Settings.DrawingSettings
        ...
        private static bool ApplyCivilCoordinateSystem(
            ...
            out string message)
        {
            ...
            try
            {
                ...
                civilDocument.Settings.DrawingSettings
                    .UnitZoneSettings.CoordinateSystemCode = code;
            ...
            IList<Point3d> boundary)
        {
            var info = new SurveyLocationInfo
            {
                ...
                BoundaryWgs84 = new List<Point3d>()
            };

            Point3d centreGeo;
            List<Point3d> boundaryGeo;
            if (TryDrawingToWgs84(
                database,
                centroid,
                boundary,
                out centreGeo,
                out boundaryGeo))
            {
                ...
                if (string.IsNullOrWhiteSpace(info.Town))
                    info.Town = "Location lookup unavailable";
            }
            else
            {
            ...
            out List<Point3d> boundaryGeo)
        {
            ...
            try
            {
                ...
                {
                    GeoLocationData geo = transaction.GetObject(
                        geoId,
                        OpenMode.ForRead,
                        false) as GeoLocationData;
        ...
        private static bool TryWgs84ToDrawing(
            Database database,
            IList<Point3d> geoPoints,
            out Point3d[] drawingPoints)
        {
        ...
        {
            string name = SecurityElement.Escape(
                string.IsNullOrWhiteSpace(info.Town)
                    ? "CE Survey Boundary"
                    : info.Town + " - Survey Boundary");
            var coordinates = new StringBuilder();
            foreach (Point3d point in info.BoundaryWgs84)
            {
                coordinates.AppendFormat(
                    CultureInfo.InvariantCulture,
                    "{0:R},{1:R},0 ",
                    point.X,
                    point.Y);
            ...
            return
                ...
                info.CentreWgs84.Y.ToString("R", CultureInfo.InvariantCulture) +
                ",0</coordinates></Point></Placemark>" +
                "<Placemark><name>Survey boundary</name>" +
                "<Style><LineStyle><width>3</width></LineStyle>" +
        ...
        private static void OpenGoogleEarth(string kmlPath, Point3d centreWgs84)
        {
            try
            {
                Process.Start(new ProcessStartInfo(kmlPath)
                {
            ...
            out string error)
        {
            ...
            try
            {
                    ...
                    StringComparison.OrdinalIgnoreCase))
                {
                    using (ZipArchive archive = ZipFile.OpenRead(sourcePath))
                    {
                        ...
                        {
                            error = "The GroundOverlay points to an online image. Save/export it as a KMZ with the image embedded, then import that KMZ.";
                            return false;
                        }
                        ...
                        ZipArchiveEntry imageEntry = archive.Entries.FirstOrDefault(entry =>
                            ...
                            archive.Entries.FirstOrDefault(entry =>
                                string.Equals(
                                    ...
                                    Path.GetFileName(normalizedHref),
                                    StringComparison.OrdinalIgnoreCase));
                        if (imageEntry == null)
                        {
                ...
                else
                {
                    ...
                    if (Uri.IsWellFormedUriString(href, UriKind.Absolute))
                    {
                        error = "The GroundOverlay points to an online image. Use a local image/KML or save it as a KMZ with the image embedded.";
                        return false;
                    }

                    imagePath = Path.GetFullPath(Path.Combine(
            ...
            string cornerError;
            if (!TryReadGroundOverlayCorners(
                kmlText,
                out corners,
                out cornerError))
            {
        ...
        private static string ReadGroundOverlayHref(string kmlText)
        {
            try
            {
                ...
                xml.LoadXml(kmlText);
                XmlNode groundOverlay = FindFirstByLocalName(xml, "GroundOverlay");
                if (groundOverlay == null) return string.Empty;
                XmlNode icon = FindFirstByLocalName(groundOverlay, "Icon");
                XmlNode href = icon == null
                    ? null
                    : FindFirstByLocalName(icon, "href");
                return href == null ? string.Empty : href.InnerText.Trim();
            }
            catch { return string.Empty; }
        }

        private static bool TryReadGroundOverlayCorners(
            string kmlText,
            out List<Point3d> corners,
            out string error)
        {
            ...
            try
            {
                ...
                XmlNode quad = FindFirstByLocalName(groundOverlay, "LatLonQuad");
                if (quad != null)
                {
                    XmlNode coordinatesNode = FindFirstByLocalName(quad, "coordinates");
                    if (coordinatesNode != null)
                    {
                        foreach (string token in Regex.Split(
                            coordinatesNode.InnerText.Trim(),
                            "\\s+"))
                        {
                ...
                XmlNode box = FindFirstByLocalName(groundOverlay, "LatLonBox");
                if (box == null)
                {
                ...
                corners.Add(RotateLonLat(
                    west, south, centreLon, centreLat, rotation));
                corners.Add(RotateLonLat(
                    east, south, centreLon, centreLat, rotation));
                corners.Add(RotateLonLat(
                    east, north, centreLon, centreLat, rotation));
                corners.Add(RotateLonLat(
                    west, north, centreLon, centreLat, rotation));
                return true;
            }
            catch (System.Exception exception)
            {
        ...
        private static double ReadOptionalChildDouble(
            XmlNode parent,
            string name,
            double fallback)
        {
        ...
        private static Point3d RotateLonLat(
            double lon,
            double lat,
            double centreLon,
            double centreLat,
            double degrees)
        {
            ...
            double cosLat = Math.Max(
                1e-9,
                Math.Cos(centreLat * Math.PI / 180.0));
        ...
        private static string ExtractPersistentImage(
            ...
            Database database)
        {
            ...
            string folder = !string.IsNullOrWhiteSpace(drawingFolder) &&
                            Directory.Exists(drawingFolder)
            ...
            IList<Point3d> clipBoundary)
        {
            if (drawingCorners == null || drawingCorners.Length < 4)
                throw new InvalidOperationException(
                    "Four GroundOverlay corners are required.");
                ...
                database.TransactionManager.StartTransaction())
            {
                ...
                if (dictionary == null)
                    throw new InvalidOperationException(
                        "Raster image dictionary is unavailable.");

                var definition = new RasterImageDef
                {
                    SourceFileName = imagePath
                };
                definition.Load();
                string definitionName = UniqueDictionaryName(
                    dictionary,
                    "CE_GE_Overlay");
                ObjectId definitionId = dictionary.SetAt(
                    definitionName,
                    definition);
                ...
                if (imageX.Length < 1e-9 || imageY.Length < 1e-9)
                    throw new InvalidOperationException(
                        "The GroundOverlay transformed to a zero-size image.");
        ...
        private static void TryClipRaster(
            RasterImage raster,
            IList<Point3d> modelBoundary)
        {
            try
            {
                ...
                using (var plane = new Plane(Point3d.Origin, Vector3d.ZAxis))
                {
                    ...
                    {
                        pixels.Add(
                            point.TransformBy(modelToPixel).Convert2d(plane));
                    }
                }
                if (pixels.Count < 3) return;
                pixels.Add(pixels[0]);
                raster.SetClipBoundary(ClipBoundaryType.Poly, pixels);
                raster.IsClipped = true;
            }
            catch
            {
                // Georeferencing remains valid if the selected boundary cannot
                // be represented as a valid raster-pixel clipping polygon.
            }
        }

        private static string UniqueDictionaryName(
        ...
        private enum SurveyDialogAction
        {
            ...
            OpenAutodeskCoordinateSelector,
            OpenGoogleEarth,
            ExportKml,
            Close
        }
        ...
        private sealed class SurveyLocationForm : Form
        {
            ...
            internal string SelectedCoordinateSystem
            {
                get { return _coordinateSystem.Text; }
            }

            internal SurveyLocationForm(
                SurveyLocationInfo info,
                IEnumerable<string> coordinateSystems)
            {
                ...
                AddRow(
                    table,
                    0,
                    "Town / nearest locality",
                    new TextBox
                    {
                        ReadOnly = true,
                        Text = info.Town ?? string.Empty,
                        Dock = DockStyle.Fill
                    });
                AddRow(
                    table,
                    ...
                    new TextBox
                    {
                        ...
                        Text = string.Format(
                            CultureInfo.InvariantCulture,
                            "X {0:0.###}   Y {1:0.###}",
                            info.DrawingCentre.X,
                            info.DrawingCentre.Y),
                        Dock = DockStyle.Fill
                    });
                AddRow(
                    table,
                    ...
                    new TextBox
                    {
                        ...
                        Text = info.HasGeographicTransform
                            ? string.Format(
                                ...
                                info.CentreWgs84.Y,
                                info.CentreWgs84.X)
                            : "Not available - DWG geolocation is not resolved",
                        Dock = DockStyle.Fill
                    });
                AddRow(
                    table,
                ...
                foreach (string code in coordinateSystems ?? new string[0])
                    _coordinateSystem.Items.Add(code);
                ...
                var note = new Label
                {
                    Text = "Apply CRS changes Civil 3D's drawing coordinate-system code; it does not move survey geometry. Google Earth conversion requires valid AutoCAD geolocation data. Use Autodesk CRS if the DWG geolocation transform must be created or corrected.",
                    AutoSize = true,
                    MaximumSize = new System.Drawing.Size(500, 0),
                    Dock = DockStyle.Fill
                };
                AddRow(table, 5, string.Empty, note);
                ...
                buttons.Controls.Add(ActionButton(
                    "Apply CRS / Recalculate",
                    SurveyDialogAction.ApplyCoordinateSystem));
                buttons.Controls.Add(ActionButton(
                    "Autodesk CRS...",
                    SurveyDialogAction.OpenAutodeskCoordinateSelector));
                buttons.Controls.Add(ActionButton(
                    "Open in Google Earth",
                    SurveyDialogAction.OpenGoogleEarth));
                buttons.Controls.Add(ActionButton(
                    "Export KML",
                    SurveyDialogAction.ExportKml));
                buttons.Controls.Add(ActionButton(
                    "Close",
                    SurveyDialogAction.Close));
