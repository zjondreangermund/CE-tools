    ...
    public sealed class SewerProductionCommands
    {
        ...
        public void RefreshAlignments()
        {
            ...
            using (Transaction transaction = document.Database.TransactionManager.StartTransaction())
            {
                ...
                foreach (string handleText in networkHandles)
                {
                    ...
                    ObjectId partId = network.GetPipeIds().FirstOrDefault();
                    if (partId.IsNull)
                        partId = network.GetStructureIds().FirstOrDefault();
                    if (!partId.IsNull)
            ...
            try
            {
                using (Transaction transaction = database.TransactionManager.StartTransaction())
                {
                    ...
                    foreach (ObjectId objectId in modelSpace)
                    {
                        ...
                        if (!alignmentByBranch.TryGetValue(branchKey, out alignment))
                            continue;

                        Extents3d extents = alignment.GeometricExtents;
                        Point3d centre = new Point3d(
                            (extents.MinPoint.X + extents.MaxPoint.X) * 0.5,
                            (extents.MinPoint.Y + extents.MaxPoint.Y) * 0.5,
                            0.0);
                        double offset = settings.LabelHeight *
                            (2.0 + (labelIndex % 4) * 1.5);
                        label.UpgradeOpen();
                        label.TextHeight = settings.LabelHeight;
                        label.Location = centre + new Vector3d(0.0, offset, 0.0);
                        labelsMoved++;
                        labelIndex++;
                    }
        ...
        public void CreateProfiles()
        {
            ...
            PromptEntityOptions surfaceOptions = new PromptEntityOptions(
                "\nSelect the existing-ground surface for the sewer profiles: ");
            ...
            out int partsAdded)
        {
            ...
            using (Transaction transaction = database.TransactionManager.StartTransaction())
            {
                ...
                ObjectId bandId = ResolveStyleId(
                    civilDocument.Styles.BandSetStyles.ProfileViewBandSetStyles,
        ...
        private static void SetStyleByReflection(object civilObject, ObjectId styleId)
        {
            ...
            property.SetValue(civilObject, styleId, null);
        }
