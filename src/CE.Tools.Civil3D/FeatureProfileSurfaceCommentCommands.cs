...
namespace CETools.Civil3D
{
    ...
    public sealed class FeatureProfileSurfaceCommentCommands
    {
        ...
        public void FeatureLineReport()
        {
            ...
            {
                "Name", "Site", "Style", "Layer", "Length", "Vertices", "Min Z", "Max Z", "Colour"
            };
            ...
            using (Transaction transaction = document.Database.TransactionManager.StartTransaction())
            {
                foreach (SelectedObject selected in selection.Value)
                {
                    ...
                    rows.Add(new List<string>
                    {
                        ...
                        featureLine.Layer,
                        ReadNumber(featureLine, "Length3D", "Length").ToString("N3", CultureInfo.CurrentCulture),
        ...
        public void FeatureLineVertexLabels()
        {
            ...
            using (Transaction transaction = document.Database.TransactionManager.StartTransaction())
            {
                foreach (SelectedObject selected in selection.Value)
                {
                    ...
                    for (int index = 0; index < points.Count; index++)
                    {
                        ...
                        string pointName = featureName + "-P" + (index + 1).ToString("D3", CultureInfo.InvariantCulture);
            ...
            document.Editor.WriteMessage(
                "\nCE_FLVERTEXLABELS complete. Annotations created={0}; rejected selections={1}; output={2}.",
                created,
                rejected,
                settings.Output);
        }
        ...
        private static List<CivilObjectChoice> ReadSurfaces(Document document)
        {
            var result = new List<CivilObjectChoice>();
            CivilDocument civilDocument = CivilApplication.ActiveDocument;
            if (civilDocument == null) return result;
        ...
        private static List<CivilObjectChoice> ReadSites(Document document)
        {
            var result = new List<CivilObjectChoice>
            {
        ...
        private static CivilObjectChoice PickObject(string title, string subtitle, IList<CivilObjectChoice> choices)
        {
