...
namespace CETools.Civil3D
{
    ...
    public sealed class ParkingSkewValidationCommands
    {
            ...
            ParkingSkewSettings settings)
        {
            ...
            using (Transaction transaction = database.TransactionManager.StartTransaction())
            {
                foreach (ObjectId id in sourceIds.Distinct())
                {
                    ...
                    double shortestEdge = ShortestPolygonEdge(points);
                    double widthMillimetres = rectangle.Width / settings.DrawingUnitsPerMillimetre;
        ...
        private static double ShortestPolygonEdge(IReadOnlyList<Point2d> points)
        {
            double shortest = double.MaxValue;
            for (int index = 0; index < points.Count; index++)
            {
                double length = points[index].GetDistanceTo(points[(index + 1) % points.Count]);
            ...
            foreach (ParkingBayCandidate candidate in candidates)
            {
                rows.Add(new List<string>
                {
                    ...
                    candidate.IsCompliant
                        ? "Perpendicular width meets the configured minimum"
                        : "Perpendicular width is below the configured minimum"
                });
            }
            return rows;
        }
