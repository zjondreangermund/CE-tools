...
namespace CETools.Civil3D
{
    public sealed class August11BellmouthTrimCommands
    {
        ...
        public void TrimEdgesToBellmouthTangencies()
        {
            ...
            model.AddPositiveDouble("Grouping", "02 Tangencies", "Junction grouping distance", 35.0, "Bellmouth curves with nearby centres are treated as one junction.");
            ...
            double grouping = Math.Max(1.0, model.Double("Grouping", 35.0));
            int trimmed = 0;
            int removedPieces = 0;
            int keptPieces = 0;
        ...
        private static List<JunctionGroup> ReadJunctionGroups(BlockTableRecord space, Transaction transaction, double grouping)
        {
            var arcs = new List<ArcInfo>();
            foreach (ObjectId id in space)
            {
                ...
                arcs.Add(new ArcInfo(arc.Center, arc.StartPoint, arc.EndPoint));
            }
            var groups = new List<JunctionGroup>();
            foreach (ArcInfo arc in arcs)
            {
                JunctionGroup group = groups.FirstOrDefault(item => item.Centre.DistanceTo(arc.Centre) <= grouping);
                if (group == null)
                {
                    group = new JunctionGroup(arc.Centre);
                    groups.Add(group);
                }
                group.TangentPoints.Add(arc.Start);
                group.TangentPoints.Add(arc.End);
                group.RecalculateCentre();
            }
            return groups;
        }
        ...
        private static List<Interval> BuildTrimIntervals(Curve curve, IEnumerable<JunctionGroup> junctions, double tolerance, double total)
        {
            ...
            foreach (JunctionGroup junction in junctions)
            {
                ...
                // Use the first/last accepted tangent stations for this junction;
                // this removes exactly the section spanning its bellmouth tangencies.
        ...
        {
            internal JunctionGroup(Point3d centre) { Centre = centre; TangentPoints = new List<Point3d>(); }
            internal Point3d Centre;
            internal List<Point3d> TangentPoints;
            internal void RecalculateCentre()
            {
                if (TangentPoints.Count == 0) return;
                Centre = new Point3d(TangentPoints.Average(p => p.X), TangentPoints.Average(p => p.Y), TangentPoints.Average(p => p.Z));
            }
        }

        private struct ArcInfo
        {
            internal ArcInfo(Point3d centre, Point3d start, Point3d end) { Centre = centre; Start = start; End = end; }
            internal Point3d Centre;
